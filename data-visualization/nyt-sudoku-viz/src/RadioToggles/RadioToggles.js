  ...
  return (
    <>
      <div className={styles.radioToggles}>
        <input type="radio" id="easy" value="Easy" onClick={() => setPrefilledData(easy_grid)} defaultChecked />
        <label htmlFor="easy">Easy</label>

        <input type="radio" id="medium" value="Medium" onClick={() => setPrefilledData(med_grid)} />
        <label htmlFor="medium">Medium</label>

        <input type="radio" id="hard" value="Hard" onClick={() => setPrefilledData(hard_grid)} />
        <label htmlFor="hard">Hard</label>

        <div className="slideItem"></div>
      </div>
      <Grid colorData={prefilledData} colors={['#FFF3B0', '#FFEB75', '#FFDA00', '#C5A900', '#9B8500']} />
    </>
  )
